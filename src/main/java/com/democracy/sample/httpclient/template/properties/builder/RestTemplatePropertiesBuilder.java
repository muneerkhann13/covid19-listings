package com.democracy.sample.httpclient.template.properties.builder;

import com.democracy.sample.httpclient.constants.REST_TEMPLATE_PROPERTY;
...
public class RestTemplatePropertiesBuilder {
	...
	public RestTemplatePropertiesSpecification build() {
		...
		for (Entry<REST_TEMPLATE_PROPERTY, String> entry : propertiesMap.entrySet()) {
			...
			case PROXY_SCHEME:
				setProxyScheme(propertyValue, defaultPropertyValue);
				break;

			case PROXY_HOST:
				setProxyHost(propertyValue, defaultPropertyValue);
				break;

			case PROXY_PORT:
				integerPropValue = getIntegerPropertyValue(propertyValue, defaultPropertyValue);
				setProxyPort(integerPropValue);
				break;
	...
	private RestTemplatePropertiesBuilder setProxyScheme(String propertyValue, String defaultPropertyValue) {

		if (propertyValue != null) {

			this.properties.setProxyScheme(propertyValue);

		}
		else {

			this.properties.setProxyScheme(defaultPropertyValue);
	...
	private RestTemplatePropertiesBuilder setProxyHost(String propertyValue, String defaultPropertyValue) {

		if (propertyValue != null) {

			this.properties.setProxyHost(propertyValue);

		}
		else {

			this.properties.setProxyHost(defaultPropertyValue);
	...
	private RestTemplatePropertiesBuilder setProxyPort(int propertyValue) {

		this.properties.setProxyPort(propertyValue);
